...
{
    #region CompexTypeConverter
    public class CompexTypeConverter : ExpandableObjectConverter
    {       
        ...
        public override object ConvertFrom(ITypeDescriptorContext context, System.Globalization.CultureInfo culture, object value)
        {
            if (value.GetType().Equals(typeof(string)))
                return new Complex(value.ToString());
            else
    ...
    [TypeConverter(typeof(CompexTypeConverter))]    
    public class Complex
    {
        ...
        /// </summary>
        [DefaultValue(0), DisplayName("Real"), XmlAttribute]
        public double Re
        {
            get { return re; }
            set { re = value; }
        ...
        /// </summary>
        [DefaultValue(0), DisplayName("Imag"), XmlAttribute]
        public double Im
        {
            get { return im; }
            set { im = value; }
        ...
        /// <summary>
        /// Inits a complex number with imaginary part = 0.
        /// </summary>
        /// <param name="real">A real number.</param>
        ...
        /// <summary>
        /// Inits a complex number.
        /// </summary>
        /// <param name="imag">The real part of the number.</param>
        /// <param name="real">The imaginary part of the number.</param>
        ...
        /// <summary>
        /// Inits a complex number from string like "a+bi".
        /// </summary>
        /// <param name="s"></param>
        public Complex(string s)
        {
            string regf = @"(\d+(\.|\,)?\d*|\d*(\.|\,)?\d+)([Ee][-+]?[0-2]?\d{1,2})?";
            string regg = @"^((?<r>([-+]?" + regf + "[r]?))|(?<i>([-+]?" + regf + @")?[i])|(?<r>([-+]?" + regf + @"[r]?))(?<i>(\s?[-+]\s?" + regf + @")?[i]))$";
            
            Regex es = new Regex(regg);
            Match ma = es.Match(s);
            if (ma.Success)
            {
                string sim = ma.Groups["i"].ToString().Replace("i","").Replace(" ","");
                string sre = ma.Groups["r"].ToString();
                if (sim.Equals(String.Empty)) sim = "0";
                if (sre.Equals(String.Empty)) sre = "0";
                
                if (!(Double.TryParse(sim, out im) & Double.TryParse(sre, out re)))
                {
                    re = 0;
                    im = 0;
                }
        ...
        /// Inits a complex number.
        /// </summary>
        /// <param name="mag">The magnitude of the number.</param>
        /// <param name="angle">The phase (rad) of the number.</param>
        static public Complex FromVector(double mag, double phase)
        {
