...
namespace UKLib.Fireball.Syntax.SyntaxDocumentParsers
{
	...
	public sealed class ParseTools
	{
        ...
        public static void AddString(string Text, Row Row, TextStyle Style, Segment Segment)
        {
            if (Text == "")
                return;
            ...
            for (int i = 0; i < Text.Length; i++)
            {
                if (Buff[i] == ' ' || Buff[i] == '\t')
                {
                    ...
                    Word ws = Row.Add(Buff[i].ToString());
                    if (Buff[i] == ' ')
                    ...
                    CurrentWord.Append(Buff[i].ToString());
            }
            if (CurrentWord.Length != 0)
            {
                Word word = Row.Add(CurrentWord.ToString());
