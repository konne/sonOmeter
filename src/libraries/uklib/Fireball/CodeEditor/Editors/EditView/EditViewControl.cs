...
using UKLib.Fireball.Windows.Forms;
using UKLib.Fireball.Windows.Forms.CodeEditor.Painter;
using UKLib.Fireball.Windows.Forms.CodeEditor.TextDraw;
using ScrollEventArgs = UKLib.Fireball.Windows.Forms.IntelliMouse.ScrollEventArgs;
using ScrollEventHandler = UKLib.Fireball.Windows.Forms.IntelliMouse.ScrollEventHandler;
using UKLib.Fireball.Win32;
using System.IO;

#endregion

namespace UKLib.Fireball.Windows.Forms.CodeEditor
{
	[ToolboxItem(false)]
	public sealed class EditViewControl : SplitViewChildWidget
	{

        private Graphics _ControlGraphics = null;


        public Graphics ControlGraphics
        {
            get
            {
                return _ControlGraphics;
            }
        }
		...
		protected override void Dispose(bool disposing)
		{
			...
			try
			{
				UKLib.Debug.DebugClass.SendDebugLine(this,UKLib.Debug.DebugLevel.Red,"disposing editview");
			}
			catch
			{
			}
		...
		~EditViewControl()
		{
			...
			try
			{
				UKLib.Debug.DebugClass.SendDebugLine(this,UKLib.Debug.DebugLevel.Red,"finalizing editview");
			}
			catch
			{
			}
					...
					case IndentStyle.LastRow:
						{
							...
							int Max = System.Math.Min(indent.Length, Caret.Position.X);
		...
		private void ScrollScreen(int Amount, int speed)
		{
			try
			{
				...
				newval = System.Math.Max(newval, vScroll.Minimum);
				newval = System.Math.Min(newval, vScroll.Maximum);
		...
		private void RedrawCaret()
		{
			...
			Painter.RenderCaret(this.ControlGraphics);
			//g.Dispose();
		}
			...
			else
			{
				if (x < View.TotalMarginWidth)
				{
					...
					else
					{
                        ...
                        Stream strm = assembly.GetManifestResourceStream("UKLib.Fireball.CodeEditor.FlippedCursor.cur");
...
//				go.Owner=(Form)this.TopLevelControl;
		...
		public void ScrollIntoView()
		{
			...
			if (Caret.CurrentRow.IsCollapsedEndPart)
			{
				...
				if (x >= View.ClientAreaWidth + View.ClientAreaStart)
					hScroll.Value = System.Math.Min(hScroll.Maximum, ((x - View.ClientAreaWidth)/View.CharWidth) + 15);

				if (x < View.ClientAreaStart + 10)
					hScroll.Value = System.Math.Max(hScroll.Minimum, ((x)/View.CharWidth) - 15);
			}
			else
			{
		...
		protected override void OnHandleCreated(EventArgs e)
		{
            ...
            _ControlGraphics = this.CreateGraphics();
		}
        ...
        protected override void OnSizeChanged(EventArgs e)
        {
            ...
            _ControlGraphics = base.CreateGraphics();
        }
		...
		protected override void OnDragDrop(DragEventArgs drgevent)
		{
			if (!this.ReadOnly)
			{
				if (Document != null)
				{
					...
					if (DropStart >= SelStart && DropStart <= SelStart + System.Math.Abs(Selection.SelLength))
						DropStart = SelStart;
