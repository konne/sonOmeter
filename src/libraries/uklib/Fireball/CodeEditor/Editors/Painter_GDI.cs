...
using UKLib.Fireball.Drawing.GDI;
using UKLib.Fireball.Syntax;
using UKLib.Fireball.Windows.Forms.CodeEditor.TextDraw;

namespace UKLib.Fireball.Windows.Forms.CodeEditor.Painter
{
	/// <summary>
	...
	public class Painter_GDI : IPainter
	{
		...
		public void Resize()
		{
			ResizeCount++;
			this.InitGraphics();
			//	UKLib.Debug.DebugClass.SendDebugLine(this,UKLib.Debug.DebugLevel.Red, ("painterresize {0} {1}",ResizeCount,Control.Name);
		...
		public void InitGraphics()
		{
            ...
            //catch
            //{
            //    //UKLib.Debug.DebugClass.SendDebugLine(this,UKLib.Debug.DebugLevel.Red, ("InitGraphics failure");
            //}
...
//			{
			...
			if (Control != null)
			{
				if (Control.IsHandleCreated)
				{
					...
					GFX.StringBuffer.Font = GFX.FontNormal;
					int h = GFX.StringBuffer.MeasureTabbedString("ABC", 0).Height + this.Control._CodeEditor.RowPadding;
					...
					Control.View.RowHeight = GFX.BackBuffer.MeasureTabbedString("ABC", 0).Height + this.Control._CodeEditor.RowPadding;
...
//			}
		...
		private Size MeasureRow(Row xtr, int Count, int OffsetX)
		{
			...
			if (xtr.InQueue)
			{
				...
				int Padd = System.Math.Max(Count - xtr.Text.Length, 0);
			...
			else
			{
                ...
                int wcount = xtr.FormattedWords.Count;

                for(int i =0; i < wcount;i++)
				{
                    w = xtr.FormattedWords[i];
					...
					if (w.Text.Length + CharNo >= Count || w == xtr.FormattedWords[xtr.FormattedWords.Count - 1])
					{
						...
						int MaxChars = System.Math.Min(CharPos, w.Text.Length);
		...
		public void RenderAll()
		{
			...
			RenderAll(Control.ControlGraphics);
		...
		private void RenderAll2(Graphics g)
		{
            ...
            //catch
            //{
            //    //			UKLib.Debug.DebugClass.SendDebugLine(this,UKLib.Debug.DebugLevel.Red, ("RenderAll failure:" + x.Message );
            //}
		}
		...
		private void RenderRow(int RowIndex, int RowPos)
		{
			...
			RenderRow(Control.ControlGraphics, RowIndex, RowPos);
			//g.Dispose();
		}
            ...
            //{
                ...
                //{
					if (RowIndex < this.Control.Document.Count && RowIndex >= 0)
					{
						...
						if (SpanFound && RowIndex >= FirstSpanRow && RowIndex <= LastSpanRow && Control._CodeEditor.ScopeBackColor != Color.Transparent)
						{
		...
		private void RenderMargin(int RowIndex)
		{
			...
			if (Control.ShowGutterMargin)
			{
				...
				if (RowIndex <= Control.Document.Count - 1)
				{
					...
					if (this.Control.View.RowHeight >= Control._CodeEditor.GutterIcons.ImageSize.Height)
					{
						if (r.Bookmarked)
							Control._CodeEditor.GutterIcons.Draw(Graphics.FromHdc(bbuff.hDC), 0, 0, 1);
						if (r.Breakpoint)
							Control._CodeEditor.GutterIcons.Draw(Graphics.FromHdc(bbuff.hDC), 0, 0, 0);
					}
					else
					{
						int w = this.Control.View.RowHeight;
						if (r.Bookmarked)
							Control._CodeEditor.GutterIcons.Draw(Graphics.FromHdc(bbuff.hDC), 0, 0, w, w, 1);
						if (r.Breakpoint)
							Control._CodeEditor.GutterIcons.Draw(Graphics.FromHdc(bbuff.hDC), 0, 0, w, w, 0);
					}
					...
					if (r.Images != null)
					{
						foreach (int i in r.Images)
						{
							if (this.Control.View.RowHeight >= Control._CodeEditor.GutterIcons.ImageSize.Height)
							{
								Control._CodeEditor.GutterIcons.Draw(Graphics.FromHdc(bbuff.hDC), 0, 0, i);
							}
							else
							{
								int w = this.Control.View.RowHeight;
								Control._CodeEditor.GutterIcons.Draw(Graphics.FromHdc(bbuff.hDC), 0, 0, w, w, i);
							}
						}
					}
				}
...
//				}
				...
				//RENDER SPAN MARGIN
				if (SpanFound && Control._CodeEditor.ScopeIndicatorColor != Color.Transparent)
				{
					if (RowIndex >= FirstSpanRow && RowIndex <= LastSpanRow)
						bbuff.FillRect(Control._CodeEditor.ScopeIndicatorColor, this.Control.View.TotalMarginWidth + 14, 0, 2, Control.View.RowHeight);

					if (RowIndex == FirstSpanRow)
						bbuff.FillRect(Control._CodeEditor.ScopeIndicatorColor, this.Control.View.TotalMarginWidth + 14, 0, 4, 2);

					if (RowIndex == LastSpanRow)
						bbuff.FillRect(Control._CodeEditor.ScopeIndicatorColor, this.Control.View.TotalMarginWidth + 14, Control.View.RowHeight - 2, 4, 2);
				}
			...
			if (RowIndex <= Control.Document.Count - 1 && Control.Selection.IsValid)
			{
				...
				if (!xtr.IsCollapsed)
				{
					if ((RowIndex > Control.Selection.LogicalBounds.FirstRow) && (RowIndex < Control.Selection.LogicalBounds.LastRow))
					{
						...
						RenderBox(Control.View.TextMargin, 0, System.Math.Max(width - Control.View.ClientAreaStart, 0), Control.View.RowHeight, Invert);
					}
					else if ((RowIndex == Control.Selection.LogicalBounds.FirstRow) && (RowIndex == Control.Selection.LogicalBounds.LastRow))
					{
						int start = MeasureRow(xtr, System.Math.Min(xtr.Text.Length, Control.Selection.LogicalBounds.FirstColumn)).Width;
						int width = MeasureRow(xtr, System.Math.Min(xtr.Text.Length, Control.Selection.LogicalBounds.LastColumn)).Width - start;
					...
					{
						int width = MeasureRow(xtr, System.Math.Min(xtr.Text.Length, Control.Selection.LogicalBounds.LastColumn)).Width;
						RenderBox(Control.View.TextMargin, 0, System.Math.Max(width - Control.View.ClientAreaStart, 0), Control.View.RowHeight, Invert);
					}
		...
		private void RenderCollapsedSelection(int RowIndex, bool Invert)
		{
			...
			{
				int start = MeasureRow(xtr, System.Math.Min(xtr.Text.Length, Control.Selection.LogicalBounds.FirstColumn)).Width;
				int min = System.Math.Min(xtr.Text.Length, Control.Selection.LogicalBounds.LastColumn);
				min = System.Math.Min(min, xtr.Expansion_EndChar);
			...
			{
				int start = MeasureRow(xtr, System.Math.Min(xtr.Text.Length, Control.Selection.LogicalBounds.FirstColumn)).Width;
				int width = MeasureRow(xtr, System.Math.Min(xtr.Text.Length, xtr.Expansion_EndChar)).Width - start;
				RenderBox(Control.View.TextMargin + start - Control.View.ClientAreaStart, 0, width, Control.View.RowHeight, Invert);
			}
			...
			if (Control.Selection.LogicalBounds.FirstRow <= RowIndex && Control.Selection.LogicalBounds.LastRow >= RowIndex)
			{
				...
				if (Control.Selection.LogicalBounds.LastRow == RowIndex)
					endchar = System.Math.Min(Control.Selection.LogicalBounds.LastColumn, xtr.Text.Length);
				else
				...
				{
					int startchar = System.Math.Max(Control.Selection.LogicalBounds.FirstColumn, xtr.Expansion_StartChar);
		...
		public TextPoint CharFromPixel(int X, int Y)
		{
            ...
            //{
				...
				RowIndex = System.Math.Min(RowIndex, Control.Document.VisibleRows.Count);
				...
				RowIndex = System.Math.Max(RowIndex, 0);
				Row xtr = null;
