...
namespace DockDotNET
{
    ...
    public class DockContainer : System.Windows.Forms.Panel
    {
        ...
        /// </summary>
        [DefaultValue(4), Category("DockDotNET")]
        ...
        /// </summary>
        [DefaultValue(true), Category("DockDotNET")]
        ...
        protected override void OnControlAdded(ControlEventArgs e)
        {
            ...
            {
                DockContainer c = e.Control as DockContainer;

                // Check, if the container is a drag dummy and add the container to the internal list.
                if (!c.isDragContainer)
                    conList.Add(e.Control);

                // Check, if the container has no panels and is a document.
                if (c.DockType == DockContainerType.Document)
                {
                    // Show or hide buttons.
                    if (c.panList.Count > 0)
                        c.ShowButtons();
                    else
                        c.HideButtons();
                }
        ...
        internal void AdjustBorders()
        {
            ...
            if (this.IsPanelContainer | !removeable)
            {
        ...
        {
            if ((activePanel != null) && (activePanel.Form != null) && activePanel.Form.AllowClose)
                btnClose.Show();
            else
                btnClose.Hide();
        ...
        private void UpdateToolTip(Point pt)
        {
            ...
            for (int i = 0; i < panList.Count; i++)
            {
                ...
                {
                    if ((toolTip.GetToolTip(this) != panel.Form.Text) | (toolTip.Active == false))
                    {
        ...
        virtual internal void WriteXml(XmlTextWriter writer)
        {
            ...
            if ((panList.Count > 0) && (activePanel != null))
                writer.WriteAttributeString("active", panList.IndexOf(activePanel).ToString());
        ...
        virtual internal void ReadXml(XmlReader reader, bool dockManagerHead)
		{
			reader.Read();
			...
			while (reader.Read())
			{
				...
				if (reader.Name == "container")
				{
					...
					c.ReadXml(reader.ReadSubtree(), dockManagerHead);

                    if (c.conList.Count == 1) // one child container found - transfer it to own content
                    {
                        DockStyle style = c.Dock;

                        c = c.conList[0] as DockContainer;
					...
					else if (!c.IsEmpty || (c.DockType == DockContainerType.Document))
					{
                        if (c.conList.Count == 2) // two child containers found
                        {
                            DockContainer con0 = c.conList[0] as DockContainer;
                            DockContainer con1 = c.conList[1] as DockContainer;

                            if ((con0.DockType == DockContainerType.Document) && con0.IsEmpty &&
                                (con1.DockType == DockContainerType.Document) && con1.IsEmpty)
                            {
                                // two empty child containers - delete them
                                con0.removeable = true;
                                con1.removeable = true;
                                c.Controls.Clear();
                                c.removeable = !dockManagerHead || (c.Dock != DockStyle.Fill);
                                c.AdjustBorders();
                            }
                            else if ((con0.DockType == DockContainerType.Document) && con0.IsEmpty && con0.removeable) //(con1.DockType == DockContainerType.Document))
                            {
                                // one empty child container - delete it
                                con0.removeable = true;
                                con1.removeable = true;
                            ...
                            else if ((con1.DockType == DockContainerType.Document) && con1.IsEmpty && con1.removeable) //(con0.DockType == DockContainerType.Document))
                            {
                                // one empty child container - delete it
                                con0.removeable = true;
                                con1.removeable = true;
                            ...
                            else if ((con0.DockType == DockContainerType.Document) && con0.IsPanelContainer && (con0.Dock == DockStyle.Fill))
                            {
                                // first of the two child containers is dockmanager head - mark as not removeable if second container is tool tpye
                                con0.removeable = (con1.DockType == DockContainerType.Document) || !dockManagerHead || (c.Dock != DockStyle.Fill);
                            }
                            else if ((con1.DockType == DockContainerType.Document) && con1.IsPanelContainer && (con1.Dock == DockStyle.Fill))
                            {
                                // second of the two child containers is dockmanager head - mark as not removeable
                                con1.removeable = (con0.DockType == DockContainerType.Document) || !dockManagerHead || (c.Dock != DockStyle.Fill);
                            }
                        }
                        else if (c.IsEmpty) // empty document container
                        {
                            c.removeable = !dockManagerHead || (c.Dock != DockStyle.Fill);
                            c.AdjustBorders();
                        }
				...
				else if (reader.Name == "panel")
				{
                    ...
                    {
                        this.Controls.Add(p);
                        p.Form.InvokeVisibleChanged(EventArgs.Empty);
                    }
				}
			}
