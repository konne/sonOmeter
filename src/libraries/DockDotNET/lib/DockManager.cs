...
namespace DockDotNET
{
	...
	/// <summary>
	/// This class is derived from the <see cref="DockDotNET.DockContainer"/> class to extend its features.
	...
	public class DockManager : DockContainer
	{
		...
		/// </summary>
        [DefaultValue(DockVisualStyle.VS2005), Category("DockDotNET")]
		...
		/// </summary>
        [DefaultValue(false), Category("DockDotNET")]
        ...
        override internal void ReadXml(XmlReader reader, bool dockManagerHead)
		{
		...
		internal static void UpdateDockGuide(DockContainer target, DockEventArgs e)
		{
            ...
            Size oldSize = dockGuide.Size;
            Point oldLoc = dockGuide.Location;
            ...
            if ((dockGuide.Location != oldLoc) || (dockGuide.Size != oldSize))
        ...
        {
            Collection<DockWindow> closeList = new Collection<DockWindow>();
        ...
        public static void CloseTools()
        {
            ...
            foreach (DockPanel p in listTool)
                closeList.Add(p.Form);
							...
							if (hostType != null)
							{
								foreach (DockManager m in managerList)
									if (m.Parent.GetType().FullName == hostType)
										m.ReadXml(reader.ReadSubtree(), true);
							}
							break;
					}
				}
